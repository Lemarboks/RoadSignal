...
| Recovery point | 24 hours or better | Age of newest verified off-host PostGIS backup |
...
3. Check PostGIS and Redis health separately. A failed readiness check should remove the API from traffic while liveness remains available for diagnosis.
...
- PostGIS unavailable: stop writes, inspect storage/credentials, use the latest verified backup only after checksum/archive validation, and follow the guarded restore command in `self-hosting.md`.
- Redis unavailable: keep PostGIS authoritative, restore Redis service, and accept that events older than the retained stream may require a full state refresh.
