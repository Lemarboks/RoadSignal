...
Location is collected to detect route proximity, update trip progress, warn drivers, and coordinate an explicitly activated simulated emergency. Production policy should retain precise active-trip points only as long as operationally required (recommended: 24 hours), reduce completed histories to coarse route/area summaries for 90 days, and delete or aggregate them thereafter. Users must be able to delete trip history subject to lawful fleet retention obligations.

Web and mobile clients request foreground location only after the user presses an Enable or current-location control. Denying permission leaves manual place search and route simulation available. The MVP does not request background location permission.
