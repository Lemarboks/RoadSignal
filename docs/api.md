...
`GET /api/v1/risk/evidence` is public, read-only, and returns the packaged evaluation report used by the Settings evidence view. It exposes the deterministic baseline method, dataset provenance and digest, holdout metrics, training-gate decision, blockers, and explicit claim limitations. It never represents the synthetic evaluation as a trained safety model.
