...
|---|---|---|---|
| Browser to API | Token theft, request forgery, credential leakage | TLS, exact CORS allowlist, in-memory tokens, no credentials in URLs/storage/logs, security headers, short JWT lifetime, rotated hashed refresh tokens | XSS can access in-memory credentials; maintain CSP review and dependency scanning, then prefer HttpOnly refresh cookies on one origin |
